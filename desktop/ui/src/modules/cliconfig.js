...
async function renderCLIConfigEditor(type, files) {
  ...
  let savedListenAddr = '0.0.0.0' // Default
  try {
  ...
  let ipOptionsHtml = localIPs
    .map((ip) => {
      const label =
        ip.interface === 'localhost' ? `${ip.ip} (${t('cliConfig.localhost')})` : `${ip.ip} (${ip.interface})`
...
export async function processCLIConfig() {
  try {
    ...
    if (currentEditorType === 'claude') {
      ...
      const result = await window.go.main.App.ProcessCodexConfigWithIP(
        files['config.toml'],
        files['auth.json'],
        selectedIP,
      )
