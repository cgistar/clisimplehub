...
function updateRecentLog(request) {
  ...
  const log = {
    ...
    status: request.status === 'COMPLETED' ? 'success' : request.status === 'FAILED' ? 'error' : 'in_progress',
      ...
      if (window.go?.main?.App?.GetLogDetail) {
        ...
        if (detail) {
          log = {
            ...
            path: detail.path,
            status: detail.status === 'success' ? 'COMPLETED' : detail.status === 'in_progress' ? 'PENDING' : 'FAILED',
    ...
    if (!log) {
      ...
      if (stateLog) {
        log = {
          ...
          path: stateLog.path,
          status:
            stateLog.status === 'success' ? 'COMPLETED' : stateLog.status === 'in_progress' ? 'PENDING' : 'FAILED',
...
function syncRealtimeDurationRefresh() {
  const hasActive = Array.from(realtimeRequests.values()).some(
