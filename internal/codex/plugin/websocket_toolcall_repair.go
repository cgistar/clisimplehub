...
func websocketDownstreamSessionKey(req *http.Request) string {
	...
	if requestID := strings.TrimSpace(headerValueCaseInsensitive(req.Header, "X-Client-Request-Id")); requestID != "" {
		return requestID
	}
	if raw := strings.TrimSpace(headerValueCaseInsensitive(req.Header, "X-Codex-Turn-Metadata")); raw != "" {
		if sessionID := strings.TrimSpace(gjson.Get(raw, "session_id").String()); sessionID != "" {
			return sessionID
		}
	}
	if sessionID := strings.TrimSpace(headerValueCaseInsensitive(req.Header, "session_id")); sessionID != "" {
		return sessionID
	}
	return ""
}
